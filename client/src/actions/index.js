...
export const handleToken = token => async dispatch => {
    const response = await axios.post("http://localhost:5000/api/billing", token);
...
export const loginUser = (formValues) => async (dispatch) => {
    // getState for auth object
    const response = await axios.post('http://localhost:5000/api/auth/login', { ...formValues});
...
export const updateUser = (userId, formValues) => async dispatch => {
    ...
    const response = await axios.patch(`http://localhost:5000/api/users/${userId}`, formValues, userId);
...
export const createProject = (formValues) => async (dispatch, getState) => {
    ...
    const response = await axios.post("http://localhost:5000/api/projects", {...formValues, userId});
...
export const updateProject = (projId, formValues) => async dispatch => {
    const response = await axios.patch(`http://localhost:5000/api/projects/${projId}`, formValues);
...
export const fetchBugsByProject = (projId) => async dispatch => {
    const response = await axios.get(`http://localhost:5000/api/bugs/project/${projId}`);
...
export const createBug = (formValues, projId) => async (dispatch, getState) => {
    ...
    const response = await axios.post('http://localhost:5000/api/bugs', { ...formValues, imageUrl: uploadConfig.data.key, author, projId });
    dispatch({ type: CREATE_BUG, payload: response.data });
    history.push(`/project/${projId}`);
};

export const updateBug = (bugId, formValues) => async (dispatch, getState) => {
    const response = await axios.patch(`http://localhost:5000/api/bugs/${bugId}`, formValues);
...
export const fetchComment = (commId) => async dispatch => {
    const response = await axios.get(`http://localhost:5000/api/comments/${commId}`);
...
export const fetchCommentsByBug = (commentId) => async dispatch => {
    ...
    const response = await axios.get(`http://localhost:5000/api/comments/bug/${commentId}`);
...
export const createComment = (formValues, bugId) => async (dispatch, getState) => {
    ...
    const response = await axios.post('http://localhost:5000/api/comments', { ...formValues, author, authorEmail, bugId });
...
export const updateComment = (commentId, formValues) => async (dispatch, getState) => {
    const response = await axios.patch(`http://localhost:5000/api/comments/${commentId}`, formValues);
